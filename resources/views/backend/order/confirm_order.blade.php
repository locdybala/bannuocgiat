        ...
        <div class="col-lg-10 col-md-12">
            <div class="card shadow-sm">
                <div class="card-header bg-info text-white">
                    <h5 class="mb-0"><i class="bi bi-clipboard-check me-2"></i>Xác nhận đơn hàng</h5>
                </div>
                <div class="card-body">
                    ...
                    <form action="{{ route('order.confirm', $order->id) }}" method="POST" class="d-inline">
                        @csrf
                        <button type="submit" class="btn btn-success"><i class="bi bi-check-circle me-1"></i>Xác nhận đơn</button>
                    </form>
                    <form action="{{ route('order.cancel', $order->id) }}" method="POST" class="d-inline ms-2" onsubmit="return confirm('Bạn có chắc muốn hủy đơn hàng này?')">
                        @csrf
                        <button type="submit" class="btn btn-danger"><i class="bi bi-x-circle me-1"></i>Hủy đơn</button>
                    </form>
                    <a href="{{ route('order.index') }}" class="btn btn-outline-secondary ms-2">Quay lại</a>
                </div>
            </div>
        </div>
    </div>
