        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    @hasrole('admin')
                        <a href="{{ route('add_category') }}" class="btn btn-primary btn-sm">
                                        ...
                                        <td>
                                            @if ($category->category_status==1)
                                                <a href="{{ route('unactive_category',['id'=>$category->category_id]) }}"
                                                   class="badge bg-success text-decoration-none">Kích hoạt</a>
                                            @else
                                                <a href="{{ route('active_category',['id'=>$category->category_id]) }}"
                                                   class="badge bg-warning text-dark text-decoration-none">Không kích hoạt</a>
                                            @endif
                                        </td>
                                        @hasrole('admin')
                                        <td>
                                            <div class="d-flex gap-2">
                                                ...
                                                <form method="POST" action="{{ route('deleteCategory',['id'=>$category->category_id]) }}" onsubmit="return confirm('Bạn có muốn xóa danh mục này không?')">
                                                    @csrf
                                                    @method('delete')
